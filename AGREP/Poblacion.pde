public class Poblacion {  
  ...
  private double[] matFitness, matValorReal;       
  ...
  //***************** Rastrigin R-->R ***************** 
  private void fitnessRastrBin(int ind) {
    indEnInterv(ind);
    matFitness[ind] = 10+Math.pow(matValorReal[ind], 2)-10*Math.cos(2*Math.PI*matValorReal[ind]);
  }

  public void calificarPoblacionRastrBin() {
    ...
    for(int k=0;k<4;k++){
      println("Punto: "+matValorReal[k]+", "+matFitness[k]+ " | Intervalo:"+intervalos[k][0]+", "+intervalos[k][1]+" | Longitud: "+(intervalos[k][1]-intervalos[k][0]));
    }
  }
  //************************************funcion con representacion GRAY INICIO *********************************
  private void fitnessRastrGray(int ind) {
    indEnIntervaloGray(ind);
    matFitness[ind] = 10+Math.pow(matValorReal[ind], 2)-10*Math.cos(2*Math.PI*matValorReal[ind]);
  }

  public void calificarPoblacionRastrGray() {
  ...
  //************************************funcion con representacion GRAY FIN *********************************
  ...
  public double indEnInterv(int ind) {   
    double valorInt = -5.12+(10.24)*((double)deBinADec(ind))/Math.pow(dominio, longIndiv);
    matValorReal[ind]=valorInt;
    return valorInt;
  }
  //************************************ Conversion GRAY --> BINARIO *********************************
  public int[] deGrayABin(int ind) {
    int[] grayVec = matPoblacion[ind];
    //Comienza en 1 ya que el prmer digito no cambia, es el mismo
    //1 si dif
    //0 iguales
    for (int i = 1; i < longIndiv; i++) {
      if ( (matPoblacion[ind][i]==0  && matPoblacion[ind][i-1]==1)||(matPoblacion[ind][i]==1  && matPoblacion[ind][i-1]==0)) { //Se aplica operador NOR
        grayVec[i]=1;
      } else {
        grayVec[i]=0;
      }
  ...
  public int deBinADecGray(int[] ind) {
    int dec=0;
    for (int i=0; i<longIndiv; i++) {
      dec = dec + (int)( ind[i]*( Math.pow(2, longIndiv-i-1) ) );
    }
    return dec;
  }
  //************************************** Conversion de BInario a Decimal ******************************
  ...
  public double indEnIntervaloGray(int ind) {
    double valorInt = -5.12+(10.24)*(deBinADecGray(deGrayABin(ind))/Math.pow(dominio, longIndiv));
    matValorReal[ind]=valorInt;
    return valorInt;
  }
  //************************************** Conversion de Entero a real   *********************************
