public class Poblacion {  
  ...
  private float pMutacion = 0.01f;
  private float pCruzamiento = 0.6f;
  ...
  public Poblacion(int n, int l, int dom) {
    ...
    intervalos = new double[numIndiv][2];
    probMutar = pMutacion;
    probReprod = pCruzamiento;  //Lo mismo que la probabilidad de cruzarse
    dominio = dom;
  }
  ...
  //************************************funcion con representacion GRAY *********************************
  private void fitnessBlackBoxGray(int ind) {       
    matFitness[ind] = (1-Math.pow(5.5*indEnInterv(deBinADecGray(deGrayABin(ind)))-3.5, 2))*(Math.cos(5.5*matValorReal[ind]-3.5) + 1) + 2;
  }

  public void calificarPoblacionBlackBGray() {

    double sum=0;

    for (int i = 0; i < numIndiv; i++) {
      fitnessBlackBoxGray(i);
      sum=sum+matFitness[i];
    }
