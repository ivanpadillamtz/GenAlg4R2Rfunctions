...
final int numero_de_individuos=10;  //Tamaño de la Poblacion

/*!!!!!!!!!!   SI SE CAMBIA EL PARAMETRO longitud_de_individuo !!!!!!!!!!!!!*/
/*!!!!!!!!!!   SE DEBE CAMBIAR EN LA CLASE EsquemasEv EL ESQUEMA USADO !!!!!!!!!!!!!*/
final int longitud_de_individuo=10;  //Longitud de las cadenas
...
void setup() {
  ...
  for (int i = 0; i < nPoints; i++) {
    x=0.01*i;
    points.add(x, (1-pow(5.5*x-3.5, 2))*(cos(5.5*x-3.5)+1)+2);
  }   

  //*************************************************************************************************************
  //Crea instancia del objeto EsquemasEv
  ...
  for (int i=0; i<esquemasDEvol.poblacion.getNumIndiv(); i++) {
    ...
    soluciones.add(new GPoint(x, (1-pow(5.5*x-3.5, 2))*(cos(5.5*x-3.5)+1)+2));
  }     
